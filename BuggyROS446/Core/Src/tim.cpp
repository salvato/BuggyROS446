...
extern TIM_HandleTypeDef hSendingTimer;      // Periodic Sending Timer
                  ...
                  uint32_t odometrySamplingPeriod)
{
    ...
    hSamplingTimer.Init.RepetitionCounter = 0;
                 ...
                 uint32_t mpuSamplingPulses)
{
    ...
    hSendingTimer.Init.AutoReloadPreload = TIM_AUTORELOAD_PRELOAD_DISABLE;
    hSendingTimer.Init.RepetitionCounter = 0;
    if (HAL_TIM_Base_Init(&hSendingTimer) != HAL_OK) {
        Error_Handler();
    }
